...
food_agent = Agent(
    role="여행 맛집 추천 전문가",
    goal="여행 목적지의 대표 맛집을 추천한다. 반드시 검색 도구를 사용하여 실시간 정보를 수집해야 함.",
    backstory="여행지의 다양한 맛집 정보를 알고 있으며, 사용자의 취향에 맞는 맛집을 추천한다. 모든 추천은 검색 도구를 통해 얻은 최신 정보를 바탕으로 한다.",
    tools=[search_tool],
    llm=llm,
    verbose=True
)
...
planner = Agent(
    ...
    backstory="당신은 고객의 숨은 니즈까지 파악하는 베테랑 기획자로, "
              ...
              "모든 계획은 검색 도구를 통해 확인된 정보를 바탕으로 합니다.",
    tools=[search_tool],
    llm=llm,
    verbose=True,
...
analyst = Agent(
    ...
    backstory="당신은 수많은 리뷰 속에서 진짜 정보를 꿰뚫어 보는 데이터 분석가입니다. "
              ...
              "모든 분석은 검색 도구를 통해 확인된 최신 정보를 바탕으로 합니다.",
    tools=[search_tool],
    llm=llm,
    verbose=True,
    allow_delegation=False,
...
**중요: 반드시 검색 도구를 사용하여 실시간 맛집 정보를 수집하세요. 추측하지 말고 검색 결과만 사용하세요.**

검색해야 할 정보:
1. {destination} 맛집 추천 2024
2. {destination} 현지 맛집 리뷰
3. {destination} 인기 맛집 예약 방법
4. {destination} 맛집 위치 정보
